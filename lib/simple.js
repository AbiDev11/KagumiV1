...
`.trim()
    ...
    */
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
      ...
      const buttonMessage = {
        document: { url: "https://wa.me/+6288232154376" },
...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: '𝙺𝙰𝙶𝚄𝙼𝙸 𝙼𝚄𝙻𝚃𝙸𝙳𝙴𝚅𝙸𝙲𝙴', body: wm, sourceUrl: 'https://chat.whatsapp.com/GcwvM9MraWW63k5mmj2o1i', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
